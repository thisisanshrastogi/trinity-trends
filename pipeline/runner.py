...
def run_pipeline(
    ...
    if start_stage <= 1 <= end_stage:
        t1 = time.time()
        scored = relevance_filter(state['normalized'], state['queries'])
    ...
    if start_stage <= 2 <= end_stage:
        t2 = time.time()
        reranked = rerank(state['scored'], state['seed_query'])
        logger.info(f"  → Stage 2: {len(reranked)} items ({time.time() - t2:.1f}s)")

        # Get embeddings for dedup/cluster stages
    ...
    if start_stage <= 4 <= end_stage:
        t4 = time.time()
        all_clustered, clusters = cluster(state['deduped'], state['deduped_embeddings'])
    ...
    if start_stage <= 5 <= end_stage:
        t5 = time.time()
        extracted, token_usages_s5 = extract_signals(state['clusters'], state['seed_query'])
