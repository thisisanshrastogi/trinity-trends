...
def merge_signals(
    ...
    for i in range(n):
        for j in range(i + 1, n):
            # Only merge if similarity is high AND both have the same anomaly status
            if sim_matrix[i, j] >= config.MERGE_SIMILARITY_THRESHOLD and signals[i].is_anomaly == signals[j].is_anomaly:
    ...
    for leader_idx, group in groups.items():
        ...
        # Anomaly status is shared across the entire group
        is_anomaly = any(s.is_anomaly for s in group)
