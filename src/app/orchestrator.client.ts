...
export class OrchestratorClient {
  ...
  private expansionService = new ExpansionService(this.intentAnalyzer, [
  ...
  async runPipeline(userEmail: string, userName: string, query: string, options?: PipelineCollectionOptions): Promise<string> {
    ...
    try {
      ...
      if (startIndex <= 0 && 0 <= endIndex) {
        ...
        const intentResult = await this.intentAnalyzer.analyze(query);
      ...
      if (startIndex <= 1 && 1 <= endIndex) {
        ...
        const rawExpansion = await this.expansionService.expand(query);
      ...
      if (startIndex <= 2 && 2 <= endIndex) {
        ...
        try {
          ...
          const redditData = await this.redditCollector.collect({ 
            query: candidate.query, 
            limit: options?.reddit?.limit ?? 10,
            sort: options?.reddit?.sort,
            time: options?.reddit?.time
          });
          this.repo.saveCollectorResult({
            ...
            platform: 'reddit',
            query: candidate.query,
            resultJson: JSON.stringify(redditData),
            resultCount: redditData.length
          });
          console.log(`  -> Reddit: ${redditData.length} items collected.`);
        } catch (err: any) {
          console.error(`  -> Reddit collection failed: ${err.message}`);
        }

        // YouTube
        try {
          console.log(`  -> Collecting from YouTube...`);
          const ytData = await this.youtubeCollector.collect({ 
            query: candidate.query, 
          ...
          this.repo.saveCollectorResult({
            ...
            platform: 'youtube',
            query: candidate.query,
            resultJson: JSON.stringify(ytData),
            resultCount: ytData.length
          });
          console.log(`  -> YouTube: ${ytData.length} items collected.`);
        } catch (err: any) {
          console.error(`  -> YouTube collection failed: ${err.message}`);
        }
        ...
        try {
          ...
          const hnData = await this.hackerNewsCollector.collect({
            ...
            minPoints: options?.hackerNews?.minPoints,
            ...
            after: options?.hackerNews?.after,
          ...
          this.repo.saveCollectorResult({
            ...
            platform: 'hackerNews',
            query: candidate.query,
            resultJson: JSON.stringify(hnData),
            resultCount: hnData.length
          });
          console.log(`  -> Hacker News: ${hnData.length} items collected.`);
        } catch (err: any) {
          console.error(`  -> Hacker News collection failed: ${err.message}`);
        }
  ...
  private async exportAndRunPython(sessionId: string, seedQuery: string, startStage = 0, endStage = 9) {
    ...
    try {
       const pythonExec = fs.existsSync(path.join('pipeline', '.venv', 'bin', 'python3')) ? path.join('pipeline', '.venv', 'bin', 'python3') : 'python3';
       const cmd = `${pythonExec} -m pipeline.run --input "${inputPath}" --output "${outputPath}" --start-stage ${startStage} --end-stage ${endStage} --state-file "${stateFile}"`;
